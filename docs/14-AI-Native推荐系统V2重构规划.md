...
候选完成编排后，优化器按每道菜实际被分配的餐次计算目标份量，并在 `0.5-2.5` 倍安全区间内统一缩放其食材数量。营养与成本同步由已经解析的食材事实按比例重新计算，不采用模型声明值。这样 AI 仍负责菜品构成和做法，确定性层负责把“偏小的一人份”校准到可验证的整餐目标。
...
| `PLAN_GENERATION_MAX_TOKENS` | `16000` | 结构化餐单最大输出长度 |
