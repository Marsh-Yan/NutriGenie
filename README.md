...
> AI Native 的个性化营养规划系统：让大模型负责创作，让确定性程序负责营养、预算与安全校验。
...
前端采用浅色莫兰迪视觉体系，以克制的鼠尾草绿、雾粉与灰蓝组织首页、画像、生成进度、方案结果和后台管理页面；桌面端强调信息层级，移动端保持完整的响应式体验。
...
![NutriGenie 注册页面](screenshots/auth.png)

## 功能概览

- 用户注册、登录和个人饮食画像管理
- 浅色莫兰迪响应式界面，覆盖首页、认证、画像、生成进度、结果页和管理后台
- 根据目标、饮食类型、预算、过敏原和库存生成 AI 原生个性化餐单
- 内置 132 种标准食材及对应营养数据，并支持常见食材别名归一化
...
- 生成每日计划、营养报告、采购清单和事实核算说明；重复校验提醒会自动归类折叠
...
│  └─ package.json
├─ screenshots/            # README 使用的新版前端页面截图
